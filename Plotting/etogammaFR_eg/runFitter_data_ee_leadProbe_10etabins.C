void runFitter_data_ee_leadProbe_10etabins()
{
  ...
  */
  //------------------------------------------
  plotdir = "plots_ee_leadProbe_dataABCD_10etabins/";
  gSystem->MakeDirectory(plotdir); 
  gSystem->MakeDirectory(plotdir+"nvtx0/"); 
  gSystem->MakeDirectory(plotdir+"nvtx1/"); 
  gSystem->MakeDirectory(plotdir+"nvtx2/"); 
  ...
  resdir = "fitRes_ee_leadProbe_dataABCD_10etabins/";  
  gSystem->MakeDirectory(resdir); 
  TFile fres("fitRes_ee_dataABCD_10etabins/fitresult.root","NEW") ;
  fres.Close();
  gROOT->LoadMacro("ZeeFitter_eeLeadProbe_Data_nvtx_eta10.cc");
  loop_runfits();
  return;
}
